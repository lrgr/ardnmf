...
#
# V. Y. F. Tan and C. F�votte. Automatic relevance determination in
...
# INITIALIZE ARDNMF
################################################################################
def init_ardnmf(V, prior, n_components, init=None, random_state=None, a=100):
...
def l1_ardnmf(V, beta, tol, max_iter, W, H, a, b, verbose, random_state):
    ...
    fit = fit[:itera]
    obj = obj[:itera]
    lambdas = lambdas[:,:itera]

    # Add constant to optain true minus log posterior value
    obj = obj + (K*cst*(1-np.log(cst)));

    # Display final values
    logger.info('iter = %4i | obj = %+5.2E | rel = %4.2E (target is %4.2E)'%(itera,obj[itera-1],rel,tol))
    if itera == max_iter:
